...
def execute_query():
    server = {"host": "localhost", "port": 5441, "dbname": "master", "user": "postgres", "password": "bd123"}
    conn = None
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(get_partitioned_select(159467, 318932))
        ...
        if conn:
            conn.close()
        end_time = time.time()
        print(f"Tempo de execução para o intervalo de IDs 159467 a 318932: {end_time - start_time} segundos")
