...
def execute_query():
    server = {"host": "localhost", "port": 5443, "dbname": "master", "user": "postgres", "password": "bd123"}
    conn = None
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(get_partitioned_select(478399, 637864))
        ...
        if conn:
            conn.close()
        end_time = time.time()
        print(f"Tempo de execução para o intervalo de IDs 478399 a 637894: {end_time - start_time} segundos")
