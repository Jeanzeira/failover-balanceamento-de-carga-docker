...
def execute_query():
    server = {"host": "localhost", "port": 5440, "dbname": "master", "user": "postgres", "password": "bd123"}
    conn = None
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(get_partitioned_select(1, 637894))
        ...
        if conn:
            conn.close()
        end_time = time.time()
        print(f"Tempo de execução para o intervalo de IDs 1 a 637894: {end_time - start_time} segundos")
